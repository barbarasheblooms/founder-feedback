  ...
  return (
    <div className="hub-wrap">
      ...
      <div className="who-bar">
        ...
        <input value={addedBy} onChange={e => saveWho(e.target.value)} placeholder="Enter your name (saved automatically)" />
      </div>
        ...
        <div className="card" style={{ padding: '1rem' }}>
          ...
          <div className="chat-input-row">
            <textarea className="chat-input" value={chatInput} onChange={e => setChatInput(e.target.value)}
              ...
              placeholder="Ask about pain points, opportunities, founder profiles..." rows={1} />
            <button className="btn btn-primary" onClick={() => sendChat()} disabled={chatLoading} style={{ alignSelf: 'flex-end', padding: '10px 14px' }}>↑</button>
          </div>
        </div>
      )}
          ...
          {entries.length < 2 ? (
            ...
            <>
              <div className="btn-row" style={{ marginBottom: '1rem' }}>
                <button className="btn" onClick={generateDiagnosis} disabled={diagLoading}>🔄 Re-analyze</button>
              </div>
              ...
              {diagnosis && !diagLoading && (
                <div>
                  {diagnosis.exec_summary && <div className="diag-summary">{diagnosis.exec_summary}</div>}
                    ...
                    <div className="card">
                      ...
                      {(diagnosis.top_themes || []).map((t, i) => (
                        <div key={i} className="freq-item">
                          <div className="freq-label"><span>{t.theme}</span><span style={{ color: 'var(--text-muted)' }}>{t.mentions}x</span></div>
                          <div className="freq-bar"><div className="freq-fill" style={{ width: `${Math.min(100, Math.round(t.mentions / entries.length * 100))}%` }} /></div>
                        </div>
                      ))}
